...
# ==============================================================
# LOAD MODEL - ✅ FIXED: Correct model filename
# ==============================================================
MODEL_FILE = "model.pkl"  # ✅ Changed from "priceprediction.pkl"

if not os.path.exists(MODEL_FILE):
    st.error(f"❌ Model file '{MODEL_FILE}' not found. Make sure model.pkl is in the same directory as app.py.")
    st.stop()
...
# ==============================================================
# LOAD / SYNTHETIC DATA - ✅ FIXED: Define DATA_FILE
# ==============================================================
DATA_FILE = "data.csv"  # ✅ FIXED: Properly defined variable

def synth_data(n=400):
    """Generate synthetic car data for visualizations"""
...
if os.path.exists(DATA_FILE):
    try:
        ...
        st.warning(f"Could not load data file: {str(e)}. Using synthetic data.")
        vis_df = synth_data()
else:
    vis_df = synth_data()
